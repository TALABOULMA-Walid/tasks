...
let nomfichier="/home/ROCQ/aoste/fndoye/simulations/tasks_latences/" in
(*let nomfichier="/home/fndoye/Documents/simulations/tasks/nbprocs_varie/" in*)
let fic_result_success="/home/ROCQ/aoste/fndoye//simulations/result/multi_pro/latence/latence_heu_multi_nbproc_fixed_3_10_tasks.ml" in
...
let is= ref 0 in
  ...
  begin
   let sched=multi_proc_with_data_dependances lresult interval li costCom !input !output in
    if(sched.ischedulable=true) then 
     begin
     latence:=(float_of_int (sched.latence_tasks));
     is:=j;
 ...
 lres:=(insert_result {ab=(r_parse.load) ;ord=(!latence)} !lres);
(* Printf.fprintf oc "%d  %f\n" j !latence;*)
 done;



(*(List.length !ltaskbis)*)
 (* if(!numbsuccess!=0) then 
 lres:=(insert_result {ab=(!moy_load) ;ord=(!latence)} !lres);
