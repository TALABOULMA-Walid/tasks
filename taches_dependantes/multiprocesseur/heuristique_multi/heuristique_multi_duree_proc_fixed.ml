...
if(ta.id=4) then 
 ...
 begin
  ...
  (* if(ta.id=1) then
    Printf.printf " ////////  verif kij=%d kji=%d " k_ij k_ji ;*)
   let bi=ref 0 in
   let tdata= ref  (max_interval+1) in 
   let li=ref 0 in
   let long= Array.length ti.ltime_data_succ in
 ...
 (* if(ta.id=4) then 
  ...
  if (!b_pred=true) then 
   readyTasks:=(insert_task ta (!readyTasks));
 end
end
...
let deb= ref interval.r_min in
 if(!deb<(List.nth !l_tinput i)) then
    deb:=(List.nth !l_tinput i);

if(!latence<((List.nth !l_toutput i)-(!deb))) then
latence:=((List.nth !l_toutput i)-(List.nth !l_tinput i));
(*Printf.printf "cycle 1  debut=%d   fin=%d \n" (!deb) (List.nth !l_toutput i)*)
done;
...
(******** calcule de wcet *)
let compute_wcet ta ltask=
let wcet =ref ta.wcet in
if(ta.lpred=[]) then 
begin
for i=0 to (List.length ltask)-1 do
wcet :=((!wcet)+(List.nth ltask i).wcet);
done;
end
else
wcet :=ta.wcet ;
...
(**************** slack *******************)
let slack ti ltask =
let wcet = (compute_wcet ti ltask) in
(ti.firstActivation+ti.deadline)-(ti.firstActivation_2+wcet);;
...
let w = ref (setW ltask) in
 ...
 (*Printf.printf "tâche t%d est allouée au proc%d\n" !ta.id !ta.idpro;*)
...
while ((!w!=[]) & ((!schedulable)=true)) do
 ...
 (* Printf.printf " ---- Task task %d : \n" !ta.id ;*)
  ...
  let maxSlack = ref min_int in
  let  wr=  ref max_int in
  let indice_pro= ref 0 in 
  let r_2=ref (!ta.firstActivation) in
...
(*Printf.printf " +++++ ta%d ta.firstActivation_2=%d ta.period=%d proc=%d wrt=%d deadline_2=%d r_ta=%d\n" !ta.id !ta.firstActivation_2  !ta.period(j+1) (wrt rproc.ltask (!ta)) (!ta).deadline_2 r_ta;*)
 ...
 if (!ta.wcet<=(!ta).deadline_2) then 
     begin
   (* Printf.printf " +++++ test \n";*)
     let sla=(slack !ta rproc.ltask) in
     (*Printf.printf " ----------- t%d sur p%d slack=%d ----------- \n"!ta.id (j+1) sla;*)
       if(sla>(!maxSlack)) then 
         begin 
           maxSlack:= sla;
           indice_pro:=j ;
           r_2:=(r_ta);
...
if(!maxSlack!=min_int) then 
  begin 
 (*  Printf.printf " *** ta%d wr=%d r*=%d d*=%d \n" !ta.id !wr (!ta).firstActivation_2 (!ta).deadline_2;*)

   ta:={id=(!ta).id;firstActivation=(!ta).firstActivation;firstActivation_2=(!r_2);wcet=(!ta).wcet;period=(!ta).period;deadline=(!ta).deadline;deadline_2=((!ta).deadline-(!r_2-(!ta).firstActivation));preemption_cost=(!ta).preemption_cost;priority=(!ta).priority;ci_t=(!ta).ci_t;di_t=(!ta).di_t;bi_t=(!ta).bi_t;lpred=(!ta).lpred;lsucc=(!ta).lsucc;indice=(List.length ((!lresult.lr).(!indice_pro)).ltask);priority_t=(!ta).priority_t;lbuffer=(!ta).lbuffer;set_blocked_tasks=(!ta).set_blocked_tasks;idpro=(!indice_pro+1);ltime_data_succ=(!ta).ltime_data_succ};
...
begin
schedulable:=false;
Printf.printf "Impossible d'allouer la tâche t%d\n" !ta.id
end;
done; 
...
let nomfichier="/home/fndoye/Documents/simulations/tasks/" in
(*let fic_result_success="/home/fndoye/simulations/result/multi_pro/success/succesRatio_n.ml" in
let fic_result_success="/home/fndoye/simulations/result/multi_pro/success/succesratio_heur_multi_with_3_procs.ml" in*)
let fic_result_success="/home/fndoye/Documents/simulations/result/multi_pro/duree/proc_fixed/duree_heur_multi_with_10_34_task_sets_3_procs_exemp_n.ml" in
let oc=open_out fic_result_success in
let longueur_interval= ref 0 in
let preemption_cost=1 in
let costCom= 1 in
let numbsuccess= ref 0 in
let list_perid_diff=ref [] in
let lres= ref [] in
let ltaskbis= ref[] in
for nbproc=1 to 1 do
for k=2 to  2 do
 (*let duree_heuristique= ref 0.0 in*)
 let sum_duree= ref 0.0 in
 ...
 longueur_interval:=(interval.f_interval-interval.r_min) ;
  ...
  (* for i=0 to (Array.length lresult.lr)-1 do 
   ...
   let sched=multi_proc_with_data_dependances lresult interval li costCom !input !output in
    if(sched.ischedulable=true) then 
     ...
     sum_duree :=(!sum_duree)+.(!duree_heuristique);
...
(*  if(!numbsuccess!=0) then (*(List.length !ltaskbis)*) *)

(* lres:=(insert_result {ab=(float_of_int (!longueur_interval*(List.length !ltaskbis))) ;ord=(!duree_heuristique)} !lres);*)
 (*lres:=(insert_result {ab=(float_of_int (List.length !ltaskbis)) ;ord=(!duree_heuristique)} !lres);*)

 lres:=(insert_result {ab=(float_of_int (List.length !ltaskbis)) ;ord=(!sum_duree /. (float_of_int !numbsuccess))} !lres);

 (*lres:=(insert_result {ab=((float_of_int !longueur_interval)/.(float_of_int (List.length !ltaskbis))) ;ord=(!duree_heuristique)} !lres);*)
(*lres:=(insert_result {ab=((!moy_load/.(float_of_int !numT)));ord=((float_of_int !numbsuccess)/.(float_of_int !numT))} !lres);*)
 (*Printf.fprintf oc "%d %f %f\n"(List.length !list_perid_diff) (!moy_load/.(float_of_int !numT)) ((float_of_int !numbsuccess)/.(float_of_int !numT));*)
