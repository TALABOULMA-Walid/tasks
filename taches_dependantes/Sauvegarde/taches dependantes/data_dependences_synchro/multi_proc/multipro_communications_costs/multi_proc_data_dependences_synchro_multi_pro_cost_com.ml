...
let rec insert elem = function
  | [] -> [elem]
  | x :: l -> 
      if elem.id < x.id then elem :: x :: l else if elem.id > x then x :: insert elem l else x ::l;;*)
...
let rec insert_bis elem = function
  | [] -> [elem]
  | x :: l -> 
      if elem.period < x.period then elem ::x:: l else if elem.period > x.period then x:: insert elem l else x::l;;
...
while (!i<((List.length ltask)-1)) do
 let ta=(List.nth ltask !i) in
  let si= ref 0 in
  if(((ti.firstActivation_2-ta.firstActivation_2) mod ta.period)=0) then 
    si:=(ta.firstActivation_2+ ((ti.firstActivation_2-ta.firstActivation_2)/ta.period)*ta.period-ti.firstActivation_2)
  else
 si:=(ta.firstActivation_2+ (((ti.firstActivation_2-ta.firstActivation_2)/ta.period)+1)*ta.period-ti.firstActivation_2);

 if(((t-(!si)) mod ta.period)=0) then 
  sum:=(!sum)+((t-(!si))/ta.period)*ta.wcet 
 else
  sum:=(!sum)+(((t-(!si))/ta.period)+1)*ta.wcet;
  i:=(!i)+1
done;
...
while (!i<((List.length ltask)-1)) do
 let ta=(List.nth ltask !i) in
 if(((t) mod ta.period)=0) then 
  sum:=(!sum)+((t)/ta.period)*ta.wcet 
 else
  sum:=(!sum)+(((t)/ta.period)+1)*ta.wcet;
  i:=(!i)+1
done;
!sum;;
...
(*Printf.printf " +++++ ta%d ta.firstActivation_2=%d ta.period=%d proc=%d wrt=%d deadline_2=%d r_ta=%d\n" !ta.id !ta.firstActivation_2  !ta.period(j+1) (wrt rproc.ltask (!ta)) (!ta).deadline_2 r_ta;*)
   ...
   (*(wrt rproc.ltask (!ta))<  Printf.printf " *** ta%d wr=%d r*=%d d*=%d idproc=%d\n" !ta.id (wrt rproc.ltask (!ta)) (!ta).firstActivation_2 (!ta).deadline_2 (j+1);*)
 if (!ta.wcet<=(!ta).deadline_2) then 
     begin     
     let sla= slack !ta in
...
let nomfichier="/home/ROCQ/aoste/fndoye/simulations/tasks/" in
(*let fic_result_success="/home/ROCQ/aoste/fndoye/simulations/result/heur_multi_pro/success/succesratio_mono.ml" in*)
let fic_result_success="/home/ROCQ/aoste/fndoye/simulations/result/heur_multi_pro/success/succesratio_heur_multi_pro.ml" in
...
for k=15 to 14 do
 ...
 list_period_diff:=[];
 for j=1 to 10 do 
 Printf.printf "Iteration k=%d j=%d \n"k j;
  ...
  let  output= ref 0 in
  while (!indice>=0) do
   let tache=(List.nth ltask !indice) in
   if(tache.lsucc=[]) then
    output:=(!output)+1;
   if(tache.lpred=[]) then
    input:=(!input)+1;
   if((find_task_list_period_diff tache !list_period_diff).trouve=false) then 
    ...
    (*Printf.printf "/////// period ta=%d  ta_2=%d \n"(List.nth !list_perid_diff ((List.length !list_perid_diff)-1) ).period tache.period;*)
   indice:=(!indice)-1;
  done;
...
*)
  ...
  let sched=multi_proc_with_data_dependances lresult interval li costCom !input !output in
  let endtTime= Unix.time() in 
  if(sched.ischedulable=true) then 
   begin
    numbsuccess:=(!numbsuccess)+1;
    lat:= (sched.latence_tasks);
  ...
  numT:=j;
  Printf.printf "systemTask=%d \n"j;
(*
indice:=((List.length !list_perid_diff)-1);

 while (!indice>=0) do  
  Printf.printf "/////// +period ta=%d \n"(List.nth !list_perid_diff !indice).period ;
  indice:=(!indice)-1;
 done;

 Printf.fprintf oc "%d %f %d\n"(List.length !list_period_diff) (r_parse.load) !lat ;*)
 done;

 lres:=(insert_result {ab=((float_of_int  nbproc)/.(float_of_int (List.length !list_period_diff)));ord=((float_of_int !numbsuccess)/.(float_of_int !numT))} !lres);

 Printf.fprintf oc "%f  %f\n"((float_of_int  nbproc)/.(float_of_int (List.length !list_period_diff)))  ((float_of_int !numbsuccess)/.(float_of_int !numT)) ;

 (*Printf.printf "Succes Ratio %d/%d\n" !numbSuccess !numT;  (List.length !list_period_diff) ((!moy_load/.(float_of_int !numT)) /.(float_of_int (List.length !list_period_diff)))  (!moy_load/.(float_of_int !numT)) *)
done;
done;
(*
let lresbis= ref [] in
for i=0 to ((List.length !lres)-1) do
lresbis:=(insert_result (List.nth !lres i) !lresbis);
done;

*)
Printf.fprintf oc "\n";
for i=0 to ((List.length !lres)-1) do
Printf.fprintf oc "%f  %f\n" (List.nth !lres i).ab (List.nth !lres i).ord;
done;
