  ...
  (*Printf.printf "t=%d ti%d bi=%d\n" t (tj.id) (((bi_t ltask1 tj p.t_1 p.idSelectedTask_t t (lresult.lr)).bi_t)*k_ij) ;
    ...
    if(ndata=k_ij)||(((!ta).period>tj.period)&(ndata=k_ji)) then
     ...
     if(ndata>k_ij) then 
      ...
      if(!ta.idpro!=tj.idpro) then  
        li:=(insert (!ltime_data_s.(!j).t_read_data) !li);
...
let parse_fich_task =
let t_1=ref {id=1;firstActivation=0;firstActivation_2=0;wcet=1;period=6;deadline=6;deadline_2=6;preemption_cost=0;priority=(1.0/. 6.0);ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=0;priority_t=(1.0/. 6.0);lbuffer=[];set_blocked_tasks=[];idpro=1;ltime_data_succ=[|{idTask=3;t_read_data=12}|]} in
...
let t_3=ref {id=3;firstActivation=0;firstActivation_2=0;wcet=1;period=12;deadline=12;deadline_2=12;preemption_cost=0;priority=(1.0 /. 12.0);ci_t=0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=1;priority_t=(1.0/. 12.0);lbuffer=[];set_blocked_tasks=[];idpro=1;ltime_data_succ=[|{idTask=4;t_read_data=24};{idTask=5;t_read_data=21}|]} in
let t_4=ref {id=4;firstActivation=0;firstActivation_2=0;wcet=2;period=24;deadline=24;deadline_2=24;preemption_cost=0;priority=(1.0 /. 24.0);ci_t=0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=2;priority_t=(1.0/. 24.0);lbuffer=[];set_blocked_tasks=[];idpro=2;ltime_data_succ=[||]} in
let t_5=ref {id=5;firstActivation=10;firstActivation_2=10;wcet=1;period=6;deadline=6;deadline_2=6;preemption_cost=0;priority=(1.0 /. 12.);ci_t=0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=1;priority_t=(1.0/. 12.0 );lbuffer=[];set_blocked_tasks=[];idpro=2;ltime_data_succ=[||]} in

t_1:={id=1;firstActivation=0;firstActivation_2=0;wcet=1;period=6;deadline=6;deadline_2=6;preemption_cost=0;priority=(1.0 /. (float_of_int 6));ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[!t_3];indice=0;priority_t=(1.0/.(float_of_int 6));lbuffer=[{idBuf=1;priority_buf=(priority_buffer [!t_1;!t_3]);setTasks_may_use_b=[!t_1;!t_3]}];set_blocked_tasks=[];idpro=1;ltime_data_succ=[|{idTask=3;t_read_data=12}|]};
...
t_3:={id=3;firstActivation=0;firstActivation_2=0;wcet=1;period=12;deadline=12;deadline_2=12;preemption_cost=0;priority=(1.0 /. (float_of_int 12));ci_t=0;di_t=0;bi_t=0;lpred=[!t_1;!t_2];lsucc=[!t_4;!t_5];indice=1;priority_t=(1.0/.(float_of_int 12));lbuffer=[{idBuf=1;priority_buf=(priority_buffer [!t_1;!t_3]);setTasks_may_use_b=[!t_1;!t_3]};{idBuf=2;priority_buf=(priority_buffer [!t_2;!t_3]);setTasks_may_use_b=[!t_2;!t_3]}; {idBuf=3;priority_buf=(priority_buffer [!t_3;!t_4;!t_5]);setTasks_may_use_b=[!t_3;!t_4;!t_5]}];set_blocked_tasks=[];idpro=1;ltime_data_succ=[|{idTask=4;t_read_data=24};{idTask=5;t_read_data=21}|]} ;

t_4:={id=4;firstActivation=0;firstActivation_2=0;wcet=2;period=24;deadline=24;deadline_2=24;preemption_cost=1;priority=(1.0 /. (float_of_int 24));ci_t= 0;di_t=0;bi_t=0;lpred=[!t_3];lsucc=[];indice=2;priority_t=(1.0/.(float_of_int 24));lbuffer=[{idBuf=3;priority_buf=(priority_buffer [!t_3;!t_4;!t_5]);setTasks_may_use_b=[!t_3;!t_4;!t_5]};{idBuf=4;priority_buf=(priority_buffer [!t_4]);setTasks_may_use_b=[!t_4]}];set_blocked_tasks=[];idpro=2;ltime_data_succ=[||]} ;  
...
while (!i<((List.length ltask)-1)) do
 ...
 if(((t-ta.firstActivation_2)mod ta.period)=0) then 
  sum:=(!sum)+((t-ta.firstActivation_2)/ta.period)*ti.wcet 
 else
  sum:=(!sum)+(((t-ta.firstActivation_2)/ta.period)+1)*ta.wcet;
  i:=(!i)+1
done;
!sum;;
...
let rec insert_w elem = function
  | [] -> [elem]
  | x :: l -> 
      if (slack elem) < (slack x) then elem :: x :: l else if (slack elem) > (slack elem) then x :: insert_w elem l else x :: l;;



(*************** find **********)
let rec find_task_list ta l = 
let vbool=ref false in
let i=ref 0 in
let re=ref {idT=0;firstAct=0;trouve=false;idP=0} in
while (!i<List.length l) &(!vbool=false) do
 let ti=(List.nth l !i) in 
 if(ti.id=ta.id) then 
 begin
 vbool:=true;
 re:={idT=ti.id;firstAct=ti.firstActivation_2;trouve=true;idP=ti.idpro};
 i:=(!i)+1;
 end
else
 i:=(!i)+1;
...
while (!i<(List.length ti.lsucc )) do
 let ta=(List.nth ti.lsucc !i) in
 let trouve= ref false in
 let j=ref 0 in
  while(!j<(List.length ta.lpred))&(!trouve= false) do 
     Printf.printf "Testtttt\n"  ;
   let tk=ref (List.nth ta.lpred !j) in
      Printf.printf "pred de ta%d est tk%d \n" ta.id !tk.id ;
   if((find_task_list !tk !wb).trouve=true) then 
    trouve:=true;
   j:=(!j)+1;
  done;

  Printf.printf "bool %b ta%d lpred=%d \n" !trouve ta.id (List.length ta.lpred);
 ...
 Printf.printf " Après update W  : " ;
 for j=0 to (List.length !wb) -1 do
  let ta = List.nth !wb j in
  Printf.printf "%d " ta.id;
 done;
  Printf.printf "\n";
...
(*****************************************************************************************************************************)
let heuristique ltask nbproc interval li=
let costCom=1 in
let ltasks= ref [] in
let w = ref (setW ltask) in
 
Printf.printf " Tasks in W " ;
 for j=0 to (List.length !w) -1 do
  let ta = List.nth !w j in
  Printf.printf "%d " ta.id;
 done;
...
let lresult= ref({lr=(Array.create nbproc r_ini );sched=true;list_t=li} )in
let k=ref 0 in 

 while (!k<nbproc) & (!k< (List.length !w)) do
  ...
  for l=0 to (List.length (!ta).lpred)-1 do
    let pred= (List.nth (!ta).lpred l) in
  ...
  let p={idproc=(!k+1);phi_t=(ph);schedulable=true;idSelectedTask_t=0;idSelectedTask_t_1=0;t=0;t_1=0;next_t=0;selectedTask=(List.nth [!ta] 0)} in
...
while ((!w!=[]) & ((!schedulable)=true)) do
  ...
  Printf.printf " Test 34  \n" ;
   ...
   let r_ta=(max !rmax !ta.firstActivation_2) in
   ta:={id=(!ta).id;firstActivation=(!ta).firstActivation;firstActivation_2=r_ta;wcet=(!ta).wcet;period=(!ta).period;deadline=(!ta).deadline;deadline_2=((!ta).deadline-(r_ta-(!ta).firstActivation_2));preemption_cost=(!ta).preemption_cost;priority=(!ta).priority;ci_t=(!ta).ci_t;di_t=(!ta).di_t;bi_t=(!ta).bi_t;lpred=(!ta).lpred;lsucc=(!ta).lsucc;indice=(!ta).indice;priority_t=(!ta).priority_t;lbuffer=(!ta).lbuffer;set_blocked_tasks=(!ta).set_blocked_tasks;idpro=(!ta).idpro;ltime_data_succ=(!ta).ltime_data_succ};
...
if(!minSlack!=max_int) then 
  begin 
   ...
   (!lresult.lr).(!indice_pro)<-{proc=((!lresult.lr).(!indice_pro)).proc;ltask=(((!lresult.lr).(!indice_pro)).ltask@[!ta]); met=((!lresult.lr).(!indice_pro)).met} ;
...
(*{lr=(!lresult).lr;sched=(!schedulable);list_t=(!lresult).list_t}*)
!lresult;;
...
let lresult=(heuristique ltask nbproc interval li) in 
