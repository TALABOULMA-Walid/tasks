...
let scheduling_test_t ltask readyTasks p t next_t met r_min lresult idpro max_interval ltasks costCom=
...
if (p.idSelectedTask_t_1=(!ta).id) & (p.t_1+(!ta).ci_t<=t) then 
 begin
   ...
   while (!j<(List.length (!ta).lsucc)) do
    ...
    let tj=(List.nth ltask1 (re.ta).indice) in
    let k_ij= k_i_j (!ta).period tj.period  in
    let k_ji= k_i_j tj.period (!ta).period  in 
    let ndata= ((!bi)*k_ji)-(((bi_t ltask1 tj p.t_1 p.idSelectedTask_t t (lresult.lr)).bi_t)*k_ij) in  
   (*Printf.printf "t=%d ti%d bi=%d k_ij=%d k_ji=%d \n" t (tj.id) (((bi_t ltask1 tj p.t_1 p.idSelectedTask_t t (lresult.lr)).bi_t)*k_ij) k_ij k_ji ;
...
(((0<cit)&(cit<=dit)&(p.idSelectedTask_t!=(!ta).id) &(((!next-(!ta).firstActivation) mod !ta.period)!=0))) ||((cit=0)&(dit=0))||((0<cit)&(cit<=dit)&(p.idSelectedTask_t=(!ta).id))(((!next-(!ta).firstActivation) mod !ta.period)!=0)
*)
(* 
 if(t<=10)then
   Printf.printf "++++++++++++++++++++++++++++++++++++++++++++++++++ t=%d !next=%d (!ta).firstActivation)=%d ci_t=%d di_t=%d +++++++++++++++++++++++++\n"t !next (!ta).firstActivation !ta.ci_t (!ta).di_t;*)
(* ((cit<=dit)&((p.idSelectedTask_t=(!ta).id)||(!ta.ci_t<=(!ta).di_t)|| (cit=0)))  ((!ta.ci_t<=(!ta).di_t)&((p.idSelectedTask_t_1=(!ta).id)||(!ta.ci_t=0))||(((t-(!ta).firstActivation) mod !ta.period)!=0)) *)
  if ((cit<=dit)&((p.idSelectedTask_t=(!ta).id)||(!ta.ci_t<=(!ta).di_t)|| (cit=0)))  then (*||(((t-(!ta).firstActivation) mod !ta.period)!=0))) *)
    begin
...
{lr=lresult.lr;ltasks=ltasks;sched=(!schedulable);list_t=(!li);}
...
(*********************************)
let set_of_ready_tasks_t p ltask t tproc max_interval ltasks=
let k=ref 0 in 
...
begin
 ...
 begin
  ...
  while (!i<(List.length ta.lpred)) & (!b_pred=true) do
   ...
   let ltask1=tproc.((re.ta).idpro-1).ltask in 
   let ti=(List.nth ltask1  (re.ta).indice )in
...
(* Printf.printf "+ t=%d  task%d: data d%d is available a from tdata=%d \n"t ta.id  ti.id !tdata ;*)
 if(ta.idpro=ti.idpro)||((ta.idpro!=ti.idpro)&(t>=(!tdata))) then 
   begin     
   (*if(ta.idpro!=ti.idpro) then *)
  (* Printf.printf "ap */*/*/*/*/*/*/*/*/ proc différent task%d et task%d t=%d tdata=%d \n" ta.id ti.id t !tdata; *) 
...
let multi_proc_with_data_dependances lresult interval li costCom=
let schedulable=ref true in
(*let ph=ref (Array.create (interval.f_interval) 0) in
let p=ref {idproc=1;phi_t=(!ph);schedulable=true;idSelectedTask_t=0;idSelectedTask_t_1=0;list_t=li;t=0;t_1=0;next_t=0;selectedTask=(List.nth ltask 0)} in
let r= ref {proc=(!p);ltask=ltask; met={tasks_currently_used_buffers=[];buffer_curently_used_t=[];priority_system_t=0.0}} in*)
...
(*     k:=(!k)+1; 
 ...
 (* Printf.printf "length ltask l=%d \n" (List.length !r.ltask) ;*)
  ...
  let readyTasks=(set_of_ready_tasks_t !p  (!r).ltask !t !lresul.lr interval.f_interval !lresul.ltasks) in
(*
Printf.printf "Ready tasks\n"; 
 ...
 lresul:= (scheduling_test_t  (!r).ltask readyTasks (!p) !t !n  (!r).met interval.r_min !lresul !ind interval.f_interval !lresul.ltasks costCom);
  (*
     if(!n>((!lresul.lr.( (!r).proc.idproc-1)).proc.next_t)) then
        n:=((!lresul.lr.((!r).proc.idproc-1)).proc.next_t);*)
...
let parse_fich_task =
let t_1=ref {id=1;firstActivation=0;firstActivation_2=0;wcet=1;period=6;deadline=6;deadline_2=6;preemption_cost=0;priority=(1.0/. 6.0);ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=1;priority_t=(1.0/. 6.0);lbuffer=[];set_blocked_tasks=[];idpro=1;ltime_data_succ=[|{idTask=3;t_read_data=12}|]} in
let t_2=ref {id=2;firstActivation=0;firstActivation_2=0;wcet=1;period=24;deadline=24;deadline_2=24;preemption_cost=0;priority=(1.0 /. 6.0);ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=0;priority_t=(1.0/. 6.0);lbuffer=[];set_blocked_tasks=[];idpro=2;ltime_data_succ=[|{idTask=3;t_read_data=12};{idTask=5;t_read_data=21}|]} in
let t_3=ref {id=3;firstActivation=0;firstActivation_2=0;wcet=1;period=12;deadline=12;deadline_2=12;preemption_cost=0;priority=(1.0 /. 12.0);ci_t=0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=3;priority_t=(1.0/. 12.0);lbuffer=[];set_blocked_tasks=[];idpro=1;ltime_data_succ=[|{idTask=4;t_read_data=24};{idTask=5;t_read_data=21}|]} in
let t_4=ref {id=4;firstActivation=0;firstActivation_2=0;wcet=2;period=24;deadline=24;deadline_2=24;preemption_cost=0;priority=(1.0 /. 24.0);ci_t=0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=4;priority_t=(1.0/. 24.0);lbuffer=[];set_blocked_tasks=[];idpro=2;ltime_data_succ=[||]} in
let t_5=ref {id=5;firstActivation=10;firstActivation_2=10;wcet=1;period=6;deadline=6;deadline_2=6;preemption_cost=0;priority=(1.0 /. 12.);ci_t=0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=2;priority_t=(1.0/. 12.0 );lbuffer=[];set_blocked_tasks=[];idpro=2;ltime_data_succ=[||]} in

t_1:={id=1;firstActivation=0;firstActivation_2=0;wcet=1;period=6;deadline=6;deadline_2=6;preemption_cost=0;priority=(1.0 /. (float_of_int 6));ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[!t_3];indice=1;priority_t=(1.0/.(float_of_int 6));lbuffer=[{idBuf=1;priority_buf=(priority_buffer [!t_1;!t_3]);setTasks_may_use_b=[!t_1;!t_3]}];set_blocked_tasks=[];idpro=1;ltime_data_succ=[|{idTask=3;t_read_data=12}|]};

t_2:={id=2;firstActivation=0;firstActivation_2=0;wcet=1;period=24;deadline=24;deadline_2=24;preemption_cost=0;priority=(1.0 /. (float_of_int 6));ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[!t_3;!t_5];indice=0;priority_t=(1.0/.(float_of_int 6));lbuffer=[{idBuf=1;priority_buf=(priority_buffer [!t_1;!t_3]) ;setTasks_may_use_b=[!t_1;!t_3]}];set_blocked_tasks=[];idpro=2;ltime_data_succ=[|{idTask=3;t_read_data=12};{idTask=5;t_read_data=21}|]} ;

t_3:={id=3;firstActivation=0;firstActivation_2=0;wcet=1;period=12;deadline=12;deadline_2=12;preemption_cost=0;priority=(1.0 /. (float_of_int 12));ci_t=0;di_t=0;bi_t=0;lpred=[!t_1;!t_2];lsucc=[!t_4;!t_5];indice=3;priority_t=(1.0/.(float_of_int 12));lbuffer=[{idBuf=1;priority_buf=(priority_buffer [!t_1;!t_3]);setTasks_may_use_b=[!t_1;!t_3]};{idBuf=2;priority_buf=(priority_buffer [!t_2;!t_3]);setTasks_may_use_b=[!t_2;!t_3]}; {idBuf=3;priority_buf=(priority_buffer [!t_3;!t_4;!t_5]);setTasks_may_use_b=[!t_3;!t_4;!t_5]}];set_blocked_tasks=[];idpro=1;ltime_data_succ=[|{idTask=4;t_read_data=24};{idTask=5;t_read_data=21}|]} ;

t_4:={id=4;firstActivation=0;firstActivation_2=0;wcet=2;period=24;deadline=24;deadline_2=24;preemption_cost=1;priority=(1.0 /. (float_of_int 24));ci_t= 0;di_t=0;bi_t=0;lpred=[!t_3];lsucc=[];indice=4;priority_t=(1.0/.(float_of_int 24));lbuffer=[{idBuf=3;priority_buf=(priority_buffer [!t_3;!t_4;!t_5]);setTasks_may_use_b=[!t_3;!t_4;!t_5]};{idBuf=4;priority_buf=(priority_buffer [!t_4]);setTasks_may_use_b=[!t_4]}];set_blocked_tasks=[];idpro=2;ltime_data_succ=[||]} ;  

t_5:={id=5;firstActivation=10;firstActivation_2=10;wcet=2;period=6;deadline=6;deadline_2=6;preemption_cost=1;priority=(1.0 /. (float_of_int 12));ci_t= 0;di_t=0;bi_t=0;lpred=[!t_2;!t_3];lsucc=[];indice=2;priority_t=(1.0/.(float_of_int 12));lbuffer=[{idBuf=3;priority_buf=(priority_buffer [!t_3;!t_4;!t_5]);setTasks_may_use_b=[!t_3;!t_4;!t_5]};{idBuf=5;priority_buf=(priority_buffer [!t_5]);setTasks_may_use_b=[!t_5]}];set_blocked_tasks=[];idpro=2;ltime_data_succ=[||]};
...
while(!i<(List.length ltask))&((List.nth ltask !i).priority>=ta.priority) do
...
(************************** heuristique d'allocation des tâches **************************************************************)
(*****************************************************************************************************************************)
let heuristique ltask nbproc interval li costCom=
let ltasks= ref [] in
...
let lresult= ref({lr=(Array.create nbproc r_ini );ltasks=(!ltasks);sched=true;list_t=li} )in
 

let k=ref 0 in 
let w = ref (setW ltask) in
 while (!k<nbproc) & ((List.length !w)!=0) do
  let ph=(Array.create (interval.f_interval) 0) in 
  let ta=ref (List.nth !w 0) in
  (*Printf.printf " ---- av Task task %d : \n" (List.nth !w 0).id ;*)
  let rmax=ref (!ta).firstActivation_2 in
  for l=0 to (List.length (!ta).lpred)-1 do
    let pred= (List.nth ltask (List.nth (!ta).lpred l).indice) in
    let re=(find_task_list pred !ltasks )in (* ltasks liste des tâches qu'on vient d'ordonnancer*)
   
    let kij= ( k_i_j pred.period (!ta).period )in
    if((re.idP-1)!=(!k)) then 
      rmax:= max !rmax (re.firstAct+((kij-1)*pred.period)+(pred.wcet)+ costCom)
    else
    rmax:= max !rmax (re.firstAct+((kij-1)*pred.period)+(pred.wcet));
  done;

  ta:={id=(!ta).id;firstActivation=(!ta).firstActivation;firstActivation_2=(!rmax);wcet=(!ta).wcet;period=(!ta).period;deadline=(!ta).deadline;deadline_2=((!ta).deadline-(!rmax-(!ta).firstActivation_2));preemption_cost=(!ta).preemption_cost;priority=(!ta).priority;ci_t=(!ta).ci_t;di_t=(!ta).di_t;bi_t=(!ta).bi_t;lpred=(!ta).lpred;lsucc=(!ta).lsucc;indice=(List.length ((!lresult.lr).(!k)).ltask);priority_t=(!ta).priority_t;lbuffer=(!ta).lbuffer;set_blocked_tasks=(!ta).set_blocked_tasks;idpro=(!k+1);ltime_data_succ=(!ta).ltime_data_succ};

  ltasks:=(insert_task !ta !ltasks);

  let p={idproc=(!k+1);phi_t=(ph);schedulable=true;idSelectedTask_t=0;idSelectedTask_t_1=0;t=0;t_1=0;next_t=0;selectedTask=(!ta)} in
  let r={proc=(p);ltask=[!ta]; met={tasks_currently_used_buffers=[];buffer_curently_used_t=[];priority_system_t=0.0}} in
  !lresult.lr.(!k)<-r;
   lresult:={lr=(!lresult.lr);ltasks=(!ltasks);sched=true;list_t=li};

  (*Printf.printf " Tasks in W %d : " !k ;
 for j=0 to (List.length !w) -1 do
  let ta = List.nth !w j in
  Printf.printf "%d " ta.id;
 done;
  Printf.printf "\n";*)
    w:= (update_w  !w !ta ltask !ltasks) ;
   k:=(!k+1);
...
while ((!w!=[]) & ((!schedulable)=true)) do
  let ta = ref (List.nth !w 0) in
 (* Printf.printf " ---- Task task %d : \n" !ta.id ;*)

  let minSlack = ref max_int in
  let  wr=  ref max_int in
  let indice_pro= ref 0 in 
  for j=0 to  (nbproc-1)  do
   let rproc=(!lresult.lr.(j)) in
   let rmax=ref min_int in
   for l=0 to (List.length !ta.lpred)-1 do
    let pred= (List.nth ltask  (List.nth !ta.lpred l).indice) in
    let re=(find_task_list pred !ltasks) in
   (*  Printf.printf " ** pred de ta%d est ta.id=%d ta.firstActivation=%d t.firstActivation_2=%d   \n" !ta.id pred.id pred.firstActivation pred.firstActivation_2  ;*)
    let kij= (k_i_j pred.period !ta.period) in
    if((re.idP-1)!=j) then 
      rmax:= max !rmax (re.firstAct+((kij-1)*pred.period)+(pred.wcet)+ costCom)
    else
   ...
   ta:={id=(!ta).id;firstActivation=(!ta).firstActivation;firstActivation_2=r_ta;wcet=(!ta).wcet;period=(!ta).period;deadline=(!ta).deadline;deadline_2=((!ta).deadline-(r_ta-(!ta).firstActivation));preemption_cost=(!ta).preemption_cost;priority=(!ta).priority;ci_t=(!ta).ci_t;di_t=(!ta).di_t;bi_t=(!ta).bi_t;lpred=(!ta).lpred;lsucc=(!ta).lsucc;indice=(!ta).indice;priority_t=(!ta).priority_t;lbuffer=(!ta).lbuffer;set_blocked_tasks=(!ta).set_blocked_tasks;idpro=(!ta).idpro;ltime_data_succ=(!ta).ltime_data_succ};
   
(*Printf.printf " +++++ ta%d ta.firstActivation_2=%d ta.period=%d proc=%d wrt=%d deadline_2=%d r_ta=%d\n" !ta.id !ta.firstActivation_2  !ta.period(j+1) (wrt rproc.ltask (!ta)) (!ta).deadline_2 r_ta;*)

   (*Printf.printf " *** ta%d wr=%d r*=%d d*=%d idproc=%d\n" !ta.id (wrt rproc.ltask (!ta)) (!ta).firstActivation_2 (!ta).deadline_2 (j+1);*)
 if ((wrt rproc.ltask (!ta))<=(!ta).deadline) then 
     begin
     
     let sla= slack !ta in
       if(sla<(!minSlack)) then 
         begin 
           minSlack:= sla;
           indice_pro:=j ;
           wr:= (wrt rproc.ltask (!ta))
         end
     end

 done;
  
if(!minSlack!=max_int) then 
  begin 
 (*  Printf.printf " *** ta%d wr=%d r*=%d d*=%d \n" !ta.id !wr (!ta).firstActivation_2 (!ta).deadline_2;*)

   ta:={id=(!ta).id;firstActivation=(!ta).firstActivation;firstActivation_2=(!ta).firstActivation_2;wcet=(!ta).wcet;period=(!ta).period;deadline=(!ta).deadline;deadline_2=(!ta).deadline_2;preemption_cost=(!ta).preemption_cost;priority=(!ta).priority;ci_t=(!ta).ci_t;di_t=(!ta).di_t;bi_t=(!ta).bi_t;lpred=(!ta).lpred;lsucc=(!ta).lsucc;indice=(List.length ((!lresult.lr).(!indice_pro)).ltask);priority_t=(!ta).priority_t;lbuffer=(!ta).lbuffer;set_blocked_tasks=(!ta).set_blocked_tasks;idpro=(!indice_pro+1);ltime_data_succ=(!ta).ltime_data_succ};
   
   (!lresult.lr).(!indice_pro)<-{proc=((!lresult.lr).(!indice_pro)).proc;ltask=(add_tri_priority  (((!lresult.lr).(!indice_pro)).ltask) !ta); met=((!lresult.lr).(!indice_pro)).met} ;
   (* Printf.printf " sched task  %d \n" !ta.id ;*)
   ltasks:=(insert_task !ta !ltasks);
   lresult:={lr=(!lresult.lr);ltasks=(!ltasks);sched=true;list_t=li};
...
{lr=(!lresult).lr;ltasks=(!ltasks);sched=(!schedulable);list_t=(!lresult).list_t};;
(*!lresult;;*)
...
let nomfichier="/home/falou/simulations/tasks/setOfTask" in
...
let costCom= 0 in
let nbproc=3 in
let nomfic=(nomfichier)^(string_of_int l)^".ml" in
let ltask= parse_fic_tasks nomfic preemption_cost in

(*Printf.printf "Résultat parse fic len=%d\n"(List.length ltask) ;
for i=0 to (List.length ltask) -1 do
let ti = List.nth ltask i in

Printf.printf "id=%d r=%d C=%d T=%d D=%d indice=%d\n" ti.id ti.firstActivation ti.wcet ti.period ti.deadline ti.indice;
   ...
   k:=  0;
   Printf.printf "\n";
  Printf.printf "****Les buffers de t%d :" ti.id;
  while !k<(List.length ti.lbuffer) do
   let bi= (List.nth ti.lbuffer !k) in
    Printf.printf "b%d:( " bi.idBuf;
    let l= ref 0 in
    while !l<(List.length bi.setTasks_may_use_b) do
      Printf.printf "t%d " (List.nth bi.setTasks_may_use_b !l).id;
      l:=(!l)+1;
    done;
    Printf.printf ") ";
    k:=(!k)+1;
...
let lresult=(heuristique ltask nbproc interval li costCom ) in 
...
let startTime= Unix.time() in
let sched=multi_proc_with_data_dependances lresult interval li costCom in
let endtTime= Unix.time() in 
if(sched=true) then 
...
Printf.printf "Succes Ratio %d/5\n" !numbSuccess;
