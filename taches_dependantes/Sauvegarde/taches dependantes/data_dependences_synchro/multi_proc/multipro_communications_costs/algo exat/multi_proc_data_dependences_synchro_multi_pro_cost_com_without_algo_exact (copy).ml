...
(****  affichage resultat d'ordonnancement******)
...
(*****************************************************************************************************************************)
let algo_exact ltask nbproc interval li costCom preemption_cost=
...
for ind=0 to (List.length !lsolution)-1  do
 let lresult=(List.nth !lsolution ind) in
 let result_multi_sched=multi_proc_with_data_dependances lresult interval li costCom in
...
let  indice=ref 0  in
while (List.nth ltask !indice).lpred=[] do
 nbpred:=(!nbpred)+1;
 indice:=(!indice)+1;
done;

indice:=(List.length ltask)-1;
let nbsucc= ref 0 in

while (List.nth ltask !indice).lsucc=[] do
 nbsucc:=(!nbsucc)+1;
 indice:=(!indice)+1;
done;*)
...
(algo_exact  ltask nbproc interval li costCom preemption_cost)
