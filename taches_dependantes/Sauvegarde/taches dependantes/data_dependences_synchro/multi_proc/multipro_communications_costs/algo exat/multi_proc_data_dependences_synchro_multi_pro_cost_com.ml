...
if (p.idSelectedTask_t_1=(!ta).id) & (p.t_1+(!ta).ci_t<=t) then 
 ...
 if(!ta.lsucc=[]) then 
  ...
  let li=ref 0 in
  while (!li<(List.length !ta.lpred)) & (!b_pred=true) do
   let re= (find_task_list (List.nth !ta.lpred !li) ltasks) in
 ...
 if(!ta.idpro=ti.idpro)||((!ta.idpro!=ti.idpro)&(t>=(!tdata))) then 
   begin     
    ...
    b:=(((bi_t ltask ti p.t_1 p.idSelectedTask_t_1 t lresult.lr).bi_t)*nk_ji)-((!bi)*nk_ij);
    if (!b!=nk_ij) then 
      b_pred:=false ;
  end
  else
  b_pred:=false ;
   ...
   if((t-lresult.t_begin)>(!latence))&(!b_pred=true) then
    begin
     t_begin:=t;
     latence:=(t-lresult.t_begin);
    end 
   else
   latence:=(!latence); 
  end
...
{lr=lresult.lr;ltasks=ltasks;sched=(!schedulable);list_t=(!li);latence=(!latence);t_begin=(!t_begin)}
...
let lresult= ref({lr=(Array.create nbproc r_ini );ltasks=(!ltasks);sched=true;list_t=li;latence=0;t_begin=interval.r_min} )in
...
let w = ref (setW ltask) in
 while (!k<nbproc) & ((List.length !w)!=0) do
  ...
  !lresult.lr.(!k)<-r;
   lresult:={lr=(!lresult.lr);ltasks=(!ltasks);sched=true;list_t=li;latence=0;t_begin=interval.r_min};
...
{lr=(!lresult).lr;ltasks=(!ltasks);sched=(!schedulable);list_t=(!lresult).list_t;latence=0;t_begin=interval.r_min};;
(*!lresult;;*)
...
let costCom= 1 in
let nbproc=2 in
let nomfic=(nomfichier)^(string_of_int 1)^".ml" in
