...
(***************************************** buffers**********************************************)
let rec bi_t ltask ta t_1 idSelectedTask_t_1 t tproc=
...
begin
 ...
 while (!i<(List.length ta.lsucc)) & (!b_succ=true) do
  ...
  (*for i=0 to (List.length ltask2) -1 do
 let ta = List.nth ltask2 i in
Printf.printf "ltask2 ta%d \n" ta.id ;
 done;*)
 (* Printf.printf "Test succ idproc=%d length=%d \n" tproc.(tsucc.idpro-1).proc.idproc (List.length ltask2);*)
...
(* Printf.printf "Test//// t=%d id=%d b_%d=%d\n" t tj.id tj.id (bi_t tj t_1 idSelectedTask_t_1 t).bi_t;*)
 let bk= ((bi_t ltask tj t_1 idSelectedTask_t_1 t tproc).bi_t) in  
...
let scheduling_test_t ltask readyTasks p t next_t met r_min lresult idpro=
...
(*********************************)
let set_of_ready_tasks_t p ltask t tproc =
let k=ref 0 in
...
begin
 ...
 while (!i<(List.length ta.lsucc)) & (!b_succ=true) do
  ...
  let k_ij= k_i_j ta.period tj.period  in
  let k_ji= k_i_j tj.period ta.period  in 
 (*Printf.printf "ta."ta.od=%d tj.period=%d k_ij=%d kji=%d \n" ta.period tj.period  k_ij k_ji;*)

  let bj=((bi_t ltask ta p.t p.idSelectedTask_t t tproc).bi_t*k_ji)-(((bi_t ltask tj p.t p.idSelectedTask_t t tproc).bi_t)*k_ij) in  (*mod k_ji*)
 ...
 Printf.printf "idPred de Task%d est Task%d\n" ta.id ti.id;*)
   ...
   let bi=(((bi_t ltask ti p.t p.idSelectedTask_t t tproc).bi_t)*k_ji)-((bk)*k_ij) in (*/k_ji*)
...
let ind =ref 0 in
 ...
 while (!ind<(Array.length !lresul.lr)) do
  let r= ref ((!lresul.lr).(!ind)) in
  Printf.printf "****** Processeur %d\n" (!r).proc.idproc;
  let p=ref (!r).proc in
  let readyTasks=(set_of_ready_tasks_t !p  (!r).ltask !t !lresul.lr) in

 (* Printf.printf "nextt %d \n" !n ;*)
  p:=(phi_t (!r).proc !t readyTasks (!r).met);
 lresul:= (scheduling_test_t  (!r).ltask readyTasks (!p) !t !n  (!r).met interval.r_min !lresul !ind );
...
let t_1=ref {id=1;firstActivation=0;wcet=1;period=6;deadline=6;preemption_cost=0;priority=(1.0/. 6.0);ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=0;priority_t=(1.0/. 6.0);lbuffer=[];set_blocked_tasks=[];idpro=1} in
let t_2=ref {id=2;firstActivation=0;wcet=1;period=24;deadline=24;preemption_cost=0;priority=(1.0 /. 6.0);ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=0;priority_t=(1.0/. 6.0);lbuffer=[];set_blocked_tasks=[];idpro=2} in
let t_3=ref {id=3;firstActivation=0;wcet=1;period=12;deadline=12;preemption_cost=0;priority=(1.0 /. 12.0);ci_t=0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=2;priority_t=(1.0/. 12.0);lbuffer=[];set_blocked_tasks=[];idpro=1} in
let t_4=ref {id=4;firstActivation=0;wcet=0;period=24;deadline=24;preemption_cost=0;priority=(1.0 /. 24.0);ci_t=0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=1;priority_t=(1.0/. 24.0);lbuffer=[];set_blocked_tasks=[];idpro=2} in
let t_5=ref {id=5;firstActivation=9;wcet=1;period=6;deadline=6;preemption_cost=0;priority=(1.0 /. 12.);ci_t=0;di_t=0;bi_t=0;lpred=[];lsucc=[];indice=1;priority_t=(1.0/. 12.0 );lbuffer=[];set_blocked_tasks=[];idpro=1} in

t_1:={id=1;firstActivation=0;wcet=1;period=6;deadline=6;preemption_cost=0;priority=(1.0 /. (float_of_int 6));ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[!t_3];indice=0;priority_t=(1.0/.(float_of_int 6));lbuffer=[{idBuf=1;priority_buf=(priority_buffer [!t_1;!t_3]);setTasks_may_use_b=[!t_1;!t_3]}];set_blocked_tasks=[];idpro=1};

t_2:={id=2;firstActivation=0;wcet=1;period=24;deadline=24;preemption_cost=0;priority=(1.0 /. (float_of_int 6));ci_t= 0;di_t=0;bi_t=0;lpred=[];lsucc=[!t_3;!t_5];indice=0;priority_t=(1.0/.(float_of_int 6));lbuffer=[{idBuf=1;priority_buf=(priority_buffer [!t_1;!t_3]) ;setTasks_may_use_b=[!t_1;!t_3]}];set_blocked_tasks=[];idpro=2} ;

t_3:={id=3;firstActivation=0;wcet=1;period=12;deadline=12;preemption_cost=0;priority=(1.0 /. (float_of_int 12));ci_t=0;di_t=0;bi_t=0;lpred=[!t_1;!t_2];lsucc=[!t_4;!t_5];indice=2;priority_t=(1.0/.(float_of_int 12));lbuffer=[{idBuf=1;priority_buf=(priority_buffer [!t_1;!t_3]);setTasks_may_use_b=[!t_1;!t_3]};{idBuf=2;priority_buf=(priority_buffer [!t_2;!t_3]);setTasks_may_use_b=[!t_2;!t_3]}; {idBuf=3;priority_buf=(priority_buffer [!t_3;!t_4;!t_5]);setTasks_may_use_b=[!t_3;!t_4;!t_5]}];set_blocked_tasks=[];idpro=1} ;

t_4:={id=4;firstActivation=0;wcet=3;period=24;deadline=24;preemption_cost=1;priority=(1.0 /. (float_of_int 24));ci_t= 0;di_t=0;bi_t=0;lpred=[!t_3];lsucc=[];indice=1;priority_t=(1.0/.(float_of_int 24));lbuffer=[{idBuf=3;priority_buf=(priority_buffer [!t_3;!t_4;!t_5]);setTasks_may_use_b=[!t_3;!t_4;!t_5]};{idBuf=4;priority_buf=(priority_buffer [!t_4]);setTasks_may_use_b=[!t_4]}];set_blocked_tasks=[];idpro=2} ;  

t_5:={id=5;firstActivation=9;wcet=2;period=6;deadline=6;preemption_cost=1;priority=(1.0 /. (float_of_int 12));ci_t= 0;di_t=0;bi_t=0;lpred=[!t_2;!t_3];lsucc=[];indice=1;priority_t=(1.0/.(float_of_int 12));lbuffer=[{idBuf=3;priority_buf=(priority_buffer [!t_3;!t_4;!t_5]);setTasks_may_use_b=[!t_3;!t_4;!t_5]};{idBuf=5;priority_buf=(priority_buffer [!t_5]);setTasks_may_use_b=[!t_5]}];set_blocked_tasks=[];idpro=1};
...
let ltask1=[!t_1;!t_5;!t_3] in
let ltask2=[!t_2;!t_4] in
