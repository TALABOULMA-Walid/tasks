...
and insert elem = function
  | [] -> [elem]
  | x :: l -> 
      if ( ((float_of_int elem.wcet) /.(float_of_int  elem.period)) < ((float_of_int x.wcet) /.(float_of_int  x.period)) )then elem :: x :: l else x :: insert elem l;;
(**************************************)
...
let oc =open_out "/home/fndoye/Documents/ExactAlgoritmNew/heuristique/debugHeuristique.ml"  in
let load = ref 0.0 in 
...
for i=0 to m-1 do
  ...
  if(!wrt<=ti.wcet) then 
    wrt:=(ti.firstActivation+ti.wcet)
done;
        ...
        begin
         ...
         begin
          let load =(List.nth !p j).load +. ot.loadT in 
           schedMult:= true; 
           if ((load)<=(!minLoad)) then  (*** (1.0-.load) pour BF **** et load pour WF*)
           begin
            minLoad:= load;
            b_indice:=j;
            b_ot:=ot;
...
let load = ref 0.0 in 
let wrt= ref min_int in
let tl=(List.length t) in
for i=0 to m-1 do
  let ti=List.nth t (tl-(1+i)) in
  Printf.fprintf oc "tâche %d %d \n" i ti.period;
  p:={idproc=(i+1);t=[ti];rotask=build_otask ti;load=(float_of_int ti.wcet)/.(float_of_int ti.period)}::!p;
  load := (!load)+. ((float_of_int ti.wcet)/.(float_of_int ti.period));
  if(!wrt<=ti.wcet) then 
...
let r_m= (List.length t)-m in
let i =ref (r_m-1) in
while ((!i<r_m)  & ((!schedulable)=true)) do 
     let ta = List.nth t (r_m-(!i+1)) in 
     Printf.fprintf oc "tâche %d %d \n" !i ta.period;
     let minLoad = ref max_float in
     let b_indice = ref 0 in
     load:= (!load) +. ((float_of_int ta.wcet)/.(float_of_int ta.period));
...
(*Main *)
let main =
(*let nomficResult="/home/fndoye/ExactAlgoritmNew/results/resultHeuristiqueAmCPU_test.ml" in
let nomficResultAm="/home/fndoye/ExactAlgoritmNew/results/resultHeuristqueAmLoad_test.ml" in*)
let nomficResultSucces="/home/fndoye/Documents/ExactAlgoritmNew/results/successRatio/resultHeuristqueAmSuccessAm.ml" in
let suc=open_out  nomficResultSucces in
(*
let oc =open_out nomficResult in
let load=open_out  nomficResultAm in
let ldifferentPeriod= ref [] in*)
let setTask="/home/fndoye/Documents/ExactAlgoritmNew/setTasks/setTask" in
(*Fournir le nombre de fichier de tâches à exécuter*)
for l=1 to 10 do
(*Printf.printf "Test_l %d  \n" l;*)
let nbSuccess=ref 0 in
let moyLoad=ref 0.0 in
(*let nbDifferentPeriod =ref 0 in*)
for k= 1 to 6 do 
 (*Printf.printf "Test_k  %d\n" k;*)
 (*Printf.printf "Taskset %d\n" k;
Printf.printf "Test 1\n";*)
let nomfic= ref "/home/fndoye/ExactAlgoritmNew/setTasks/setOfTask.ml" in
(*let nomfic= ref "/home/fndoye/ExactAlgoritmNew/setTasks/harmonique/setOfTask.ml" in*)
let ltasks = ref  [] in 
...
if (r.sched==true) then 
 ...
 Printf.printf "Success  %d  %d \n" l k;*)
  ...
  (*Printf.fprintf oc "%f %f\n" !loadSetTask (endtTime-.startTime);*)
  (*print_result r;
  Printf.printf "\n";*)
 end 
done;
...
(* let x= (float_of_int !nbDifferentPeriod) /. 1.0 in *)
